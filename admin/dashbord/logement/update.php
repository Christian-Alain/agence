...
if(!empty($_POST)) 
{
    ...
    { 
        ...
        if($isImageUpdated)
        {
            $statement = $acces->prepare("UPDATE logement  set titre = ?, description = ?, prix = ?, lieu = ?, image = ? WHERE id = ?");
            $statement->execute(array($titre,$description,$prix,$lieu,$image,$id));
        }
        else
        {
            $statement = $acces->prepare("UPDATE logement  set titre = ?, description = ?, prix = ?, lieu = ? WHERE id = ?");
    ...
    {
        // $db = Database::connect();
        $statement = $acces->prepare("SELECT * FROM logement where id = ?");
