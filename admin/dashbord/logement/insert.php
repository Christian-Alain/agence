            ...
            if(strlen($description) <= 30000){
                if(isset($_FILES['file'])){
                    ...
                    if(in_array($extension, $extensions) && $size <= $maxSize && $error == 0){
                        ...
                        $uniqueName = uniqid('log-', true);
                        //uniqid génère quelque chose comme ca : 5f586bf96dcd38.73540086
                        $image = $uniqueName.".".$extension;
                        //$file = 5f586bf96dcd38.73540086.jpg

                        // telechargement de l'image puis lui attribué un nouveau nom
                        move_uploaded_file($tmpName, '../../img/'.$image);

                        // requette dans la base de donnée
                        $req = $acces->prepare('INSERT INTO logement (titre,image,prix,lieu,description) VALUES (?,?,?,?,?)');
                        $req->execute([$titre,$image,$prix,$lieu,$description]);
                        ...
                        sleep(10);
                        header("location:logement_crud.php");
                    }
                    else{
                        echo "Une erreur est survenue";
                    }
                }
            }else{$descriptionError = "La description doit etrelong";}
